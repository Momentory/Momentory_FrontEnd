...
export const updateUserProfile = async (payload: UpdateProfilePayload) => {
  try {
    const res = await api.put("/api/mypage/profile", payload, {
      headers: { "Content-Type": "application/json" },
    });
    return res.data?.result;
  } catch (err) {
...
export const getUserPosts = async (
  ...
  try {
    ...
    const res = await api.get("/api/community/users/posts", { params });
    return (
      res.data?.result ?? {
        posts: [],
        nextCursor: null,
...
export const getUserScraps = async (
  ...
  try {
    ...
    const res = await api.get("/api/community/users/scraps", { params });
    return (
      res.data?.result ?? {
        posts: [],
        nextCursor: null,
...
export const getUserLikes = async (
  ...
  try {
    ...
    const res = await api.get("/api/community/users/likes", { params });
    return (
      res.data?.result ?? {
        posts: [],
        nextCursor: null,
...
export const searchPostsBySingleTag = async (tagName: string) => {
  const res = await api.get(
    `/api/community/posts/tag/${encodeURIComponent(tagName)}`
  );

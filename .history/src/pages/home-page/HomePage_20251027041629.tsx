...
export default function HomePage() {
  ...
  // 컴포넌트가 처음 렌더링될 때 홈 화면 데이터를 불러옵니다.
  useEffect(() => {
    const fetchHomeData = async () => {
      try {
        setIsLoading(true);
        // 여러 API를 동시에 호출하여 효율적으로 데이터를 받아옵니다.
        ...
        setMarkers(map.markers || []); // API 응답에 마커 데이터가 있으면 저장
      } catch (e) {
  ...
  return (
    // 공통 하단 바 높이를 고려해 `pb-28` 같은 하단 여백은 제거합니다.
      ...
      {isUploadModalOpen && (
        ...
        >
          <div
            ...
            className="bg-white rounded-t-2xl shadow-lg w-full max-w-md p-6 relative"
          >
            <button className="w-full text-left p-4 flex items-center gap-4 text-gray-700 hover:bg-gray-50 rounded-lg">
              <img src="/icons/camera.png" alt="카메라" className="w-6 h-6" />
              <span className="font-medium">Camera</span>
            </button>
            <div className="border-t my-2"></div>
            <button className="w-full text-left p-4 flex items-center gap-4 text-gray-700 hover:bg-gray-50 rounded-lg">
              <img src="/icons/gallery.png" alt="갤러리" className="w-6 h-6" />

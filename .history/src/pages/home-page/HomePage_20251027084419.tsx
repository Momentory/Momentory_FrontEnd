...
export default function HomePage() {
  ...
  // 네비게이션의 "업로드" 클릭을 감지해서 화면 이동을 막고 모달만 띄우기
  useEffect(() => {
    const handleUploadClick = (e: MouseEvent) => {
      const target = e.target as HTMLElement;

      if (
        target.closest("a[href='/upload']") ||
        target.closest("button")?.textContent?.includes("업로드") ||
        target.textContent?.includes("업로드")
      ) {
        e.preventDefault();
        e.stopImmediatePropagation(); // 라우터로 이벤트 전파 차단
        setIsUploadModalOpen(true); // 모달 오픈
      }
    };

    document.addEventListener("click", handleUploadClick, true); // 캡처 단계에서 감지
    return () => document.removeEventListener("click", handleUploadClick, true);
  }, []);
            ...
            <div className="relative w-[220px]">
              ...
              {markers.map((m) => (
                <div
                  ...
                  style={{ top: `${m.top}px`, left: `${m.left}px`, backgroundColor: m.color }}
                ></div>
              ))}
            </div>
          </div>
          ...
          >
            <div className="flex flex-col py-2">
              ...
              >
                ...
                <span className="text-[15px] font-medium text-gray-800">Camera</span>
              </button>
              ...
              <button
                ...
                className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition"
              >
                <img src="/images/gallery.png" alt="gallery" className="w-6 h-6" />

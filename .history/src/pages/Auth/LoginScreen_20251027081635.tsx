...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("로그인 성공!");
      navigate("/auth/splash"); // 로그인 성공 시 이동할 페이지
    } catch (err) {
      console.error(err);
      ...
      <div className="w-[330px] border border-gray-300 rounded-[10px] p-6 text-center">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
          ...
          <button
            ...
            className="w-full h-[55px] bg-[#FEE500] text-[#3C1E1E] text-[16px] font-semibold rounded-[15px] mt-2 transition active:scale-95 flex items-center justify-center gap-2"
          >
            <img
